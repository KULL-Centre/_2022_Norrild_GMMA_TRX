# Copyright (C) 2017-2022 Kristoffer Enoe Johansson <kristoffer.johansson@bio.ku.dk>
...
# settings$ifs_min_prob = 1e-5 # Emma and Phillip setting
...
# The median is rather sensitive for a bimodal distribution
...
breaks = seq(min(mutant[mi_use,"signal"]), max(mutant[mi_use,"signal"]), .02)
plot(0,0, xlim=c(0,1.1), ylim=c(0,50), xlab="signal", ylab="density")
d$signal_mode = NA
for (nmut in d$n_mut) {
...
y_jitter = rnorm(length(mutant$N_sub),0,.05)
points(mutant$N_sub+jitter, mutant$signal+y_jitter, pch=".", col="red")
mask = mutant$init=="use"
points(mutant$N_sub[mask]+jitter[mask], mutant$signal[mask]+y_jitter[mask], pch=".")
...
# par(ask=T)
settings$curve_max_deviance = 0.4
settings$curve_max_stderr = 2.0
...
# Additional plots
################################################################################
# Plot distribution of 1-mutant activity
quartz(height=5, width=9)
breaks = seq(min(subst$signal,na.rm=T)-0.1, max(subst$signal,na.rm=T)+0.1, length.out=100)
h_all = hist(subst$signal, breaks=breaks, plot=F)
h_init = hist(subst[which(subst$init_m %in% init_m_fit_levels),"signal"], breaks=breaks, plot=F)
plot(0,0,xlim=c(breaks[1],breaks[length(breaks)]), ylim=c(0,max(h_all$density,h_init$density)), xlab="Single mutant signal")
lines(breaks, dnorm(breaks, mean=wt$signal, sd=wt$sd), lwd=2, col=3)
lines(h_all$mids, h_all$density, lwd=2, col=1)
lines(h_init$mids, h_init$density, lwd=2, col=2)
legend("topleft",c("All single mut.","Used single-mut","WT dist."), col=c(1,2,3), lty=1, lwd=2)
